...
TEST(HW3Test, TEST13) {
  Server server;
  User david = server.create_user("david");
  EXPECT_THROW(server.create_user("david"),
               std::logic_error); // username already exists
}
...
//     bool result = david.send_text_message("hello jenifer", "jenifer"); //
//     this time jenifer doesnt exist EXPECT_FALSE(result);
//     EXPECT_EQ(server.get_messages().size(), 0);
// }
...
//     auto msgs = server.get_all_messages_from("david"); // 'while' and 'for'
//     is not allowed in this function EXPECT_EQ(msgs.size(), 3);
...
//     auto msgs = server.get_all_messages_to("jenifer"); // 'while' and 'for'
//     is not allowed in this function EXPECT_EQ(msgs.size(), 2);
...
//     auto msgs = server.get_chat("david", "jenifer"); // 'while' and 'for' is
//     not allowed in this function EXPECT_EQ(msgs.size(), 4); TextMessage* msg1
//     = reinterpret_cast<TextMessage*>(msgs[1]); EXPECT_EQ(msg1->get_text(),
//     "hi there");
// }
...
//     Server::sort_msgs(msgs); // 'while' and 'for' is not allowed in this
//     function EXPECT_EQ(reinterpret_cast<TextMessage*>(msgs[0])->get_text(),
//     "message 1");
//     EXPECT_EQ(reinterpret_cast<TextMessage*>(msgs[1])->get_text(), "message
//     2"); EXPECT_EQ(reinterpret_cast<TextMessage*>(msgs[2])->get_text(),
//     "message 3");
//     EXPECT_EQ(reinterpret_cast<TextMessage*>(msgs[3])->get_text(), "message
//     4");
// }
